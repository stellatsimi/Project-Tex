...
SELECT prod_inv_name, ROUND(AVG(prod_inv_quantity)), prod_inv_quantity_type
FROM product_Invoice
	JOIN Invoice ON prod_inv_id = invoice_id
    JOIN supplier ON inv_supplier_id = supplier_id
WHERE supplier_name = "Κυριακόπουλος"
GROUP BY prod_inv_name;
